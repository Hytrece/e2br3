...
class RuleSourceCoverageTests(unittest.TestCase):
    ...
    def test_real_lr_sources_are_fully_covered(self) -> None:
        ...
        self.assertIn("LR", payload["auditedPages"])
        result = validate.ValidationResult()
        coverage = rule_source_coverage.validate_coverage_structure(
            root, result
        )
    ...
    def test_real_rp_sources_are_fully_covered(self) -> None:
        root = Path(__file__).resolve().parents[1]
        payload = rule_source_coverage.load_coverage(root)
        self.assertIn("RP", payload["auditedPages"])
        result = validate.ValidationResult()
        coverage = rule_source_coverage.validate_coverage_structure(
            root, result
        )
    ...
    def test_real_sd_sources_are_fully_covered(self) -> None:
        root = Path(__file__).resolve().parents[1]
        payload = rule_source_coverage.load_coverage(root)
        self.assertIn("SD", payload["auditedPages"])
        result = validate.ValidationResult()
        coverage = rule_source_coverage.validate_coverage_structure(
            root, result
        )
    ...
    def test_real_si_sources_are_fully_covered(self) -> None:
        root = Path(__file__).resolve().parents[1]
        payload = rule_source_coverage.load_coverage(root)
        self.assertIn("SI", payload["auditedPages"])
        result = validate.ValidationResult()
        coverage = rule_source_coverage.validate_coverage_structure(
            root, result
        )
