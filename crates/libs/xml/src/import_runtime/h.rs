...
pub(crate) async fn import_section_h(
	...
	let Some((narrative, sender_diagnoses, case_summaries)) =
		crate::import_sections::h_narrative::parse_h_narrative(xml)?
			.map(|narrative| {
				Ok::<_, Error>((
					narrative,
					crate::import_sections::h_narrative::parse_h_sender_diagnoses(
						xml,
					)?,
					crate::import_sections::h_narrative::parse_h_case_summaries(
						xml,
					)?,
				))
			})
...
async fn import_sender_diagnoses(
	...
	mm: &ModelManager,
	narrative_id: Uuid,
	items: Vec<crate::import_sections::h_narrative::HSenderDiagnosisImport>,
) -> Result<()> {
	for item in items {
...
async fn import_case_summaries(
	...
	mm: &ModelManager,
	narrative_id: Uuid,
	items: Vec<crate::import_sections::h_narrative::HCaseSummaryImport>,
) -> Result<()> {
	for item in items {
		let existing: Option<(Uuid,)> = mm
			.dbx()
