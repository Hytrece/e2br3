...
use lib_rest_core::{is_unique_violation, Error, Result};
			...
			Box::pin(async move {
					...
					.await
				{
					Ok(_) => {
						...
						.await?;
						CaseValidationSummaryBmc::mark_stale_for_case(
							ctx, mm, case_id,
						)
						.await?;
						let entity = SafetyReportIdentificationBmc::get_by_case(
							ctx, mm, case_id,
						)
						.await?;
						return Ok((
				...
				match SafetyReportIdentificationBmc::create(ctx, mm, data).await {
					...
					Err(err) if is_unique_violation(&err) => {
						match SafetyReportIdentificationBmc::get_by_case(
							ctx, mm, case_id,
						)
						.await
						{
	...
	lib_rest_core::with_authorized_case_child_mutation(
		...
		move |ctx, mm| {
			Box::pin(async move {
				...
				let ctx_for_update =
					if requires_nullification_compliance(ctx, mm, case_id, &data)
						.await?
					{
...
pub async fn delete_safety_report_identification(
	...
	lib_rest_core::with_authorized_case_child_mutation(
		...
		move |ctx, mm| {
			Box::pin(async move {
				SafetyReportIdentificationBmc::delete_by_case(ctx, mm, case_id)
					.await?;
				CaseValidationSummaryBmc::mark_stale_for_case(ctx, mm, case_id)
					.await?;
