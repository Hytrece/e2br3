-- Stable ISO 3166-1 alpha-2 baseline for fresh and offline installations.
-- deploy/ec2/load-iso-countries.sh remains the authoritative refresh path.

INSERT INTO iso_countries (code, name, active)
VALUES
    ('AF', 'Afghanistan', true),
    ('AL', 'Albania', true),
    ('DZ', 'Algeria', true),
    ('AS', 'American Samoa', true),
    ('AD', 'Andorra', true),
    ('AO', 'Angola', true),
    ('AI', 'Anguilla', true),
    ('AQ', 'Antarctica', true),
    ('AG', 'Antigua and Barbuda', true),
    ('AR', 'Argentina', true),
    ('AM', 'Armenia', true),
    ('AW', 'Aruba', true),
    ('AU', 'Australia', true),
    ('AT', 'Austria', true),
    ('AZ', 'Azerbaijan', true),
    ('BS', 'Bahamas (The)', true),
    ('BH', 'Bahrain', true),
    ('BD', 'Bangladesh', true),
    ('BB', 'Barbados', true),
    ('BY', 'Belarus', true),
    ('BE', 'Belgium', true),
    ('BZ', 'Belize', true),
    ('BJ', 'Benin', true),
    ('BM', 'Bermuda', true),
    ('BT', 'Bhutan', true),
    ('BO', 'Bolivia (Plurinational State of)', true),
    ('BQ', 'Bonaire, Sint Eustatius and Saba', true),
    ('BA', 'Bosnia and Herzegovina', true),
    ('BW', 'Botswana', true),
    ('BV', 'Bouvet Island', true),
    ('BR', 'Brazil', true),
    ('IO', 'British Indian Ocean Territory (the)', true),
    ('BN', 'Brunei Darussalam', true),
    ('BG', 'Bulgaria', true),
    ('BF', 'Burkina Faso', true),
    ('BI', 'Burundi', true),
    ('CV', 'Cabo Verde', true),
    ('KH', 'Cambodia', true),
    ('CM', 'Cameroon', true),
    ('CA', 'Canada', true),
    ('KY', 'Cayman Islands (the)', true),
    ('CF', 'Central African Republic (the)', true),
    ('TD', 'Chad', true),
    ('CL', 'Chile', true),
    ('CN', 'China', true),
    ('CX', 'Christmas Island', true),
    ('CC', 'Cocos (Keeling) Islands (the)', true),
    ('CO', 'Colombia', true),
    ('KM', 'Comoros (the)', true),
    ('CD', 'Congo (the Democratic Republic of the)', true),
    ('CG', 'Congo (the)', true),
    ('CK', 'Cook Islands (the)', true),
    ('CR', 'Costa Rica', true),
    ('HR', 'Croatia', true),
    ('CU', 'Cuba', true),
    ('CW', 'Curaçao', true),
    ('CY', 'Cyprus', true),
    ('CZ', 'Czechia', true),
    ('CI', 'Côte d''Ivoire', true),
    ('DK', 'Denmark', true),
    ('DJ', 'Djibouti', true),
    ('DM', 'Dominica', true),
    ('DO', 'Dominican Republic (the)', true),
    ('EC', 'Ecuador', true),
    ('EG', 'Egypt', true),
    ('SV', 'El Salvador', true),
    ('GQ', 'Equatorial Guinea', true),
    ('ER', 'Eritrea', true),
    ('EE', 'Estonia', true),
    ('SZ', 'Eswatini', true),
    ('ET', 'Ethiopia', true),
    ('FK', 'Falkland Islands (the) [Malvinas]', true),
    ('FO', 'Faroe Islands (the)', true),
    ('FJ', 'Fiji', true),
    ('FI', 'Finland', true),
    ('FR', 'France', true),
    ('GF', 'French Guiana', true),
    ('PF', 'French Polynesia', true),
    ('TF', 'French Southern Territories (the)', true),
    ('GA', 'Gabon', true),
    ('GM', 'Gambia (the)', true),
    ('GE', 'Georgia', true),
    ('DE', 'Germany', true),
    ('GH', 'Ghana', true),
    ('GI', 'Gibraltar', true),
    ('GR', 'Greece', true),
    ('GL', 'Greenland', true),
    ('GD', 'Grenada', true),
    ('GP', 'Guadeloupe', true),
    ('GU', 'Guam', true),
    ('GT', 'Guatemala', true),
    ('GG', 'Guernsey', true),
    ('GN', 'Guinea', true),
    ('GW', 'Guinea-Bissau', true),
    ('GY', 'Guyana', true),
    ('HT', 'Haiti', true),
    ('HM', 'Heard Island and McDonald Islands', true),
    ('VA', 'Holy See (the)', true),
    ('HN', 'Honduras', true),
    ('HK', 'Hong Kong', true),
    ('HU', 'Hungary', true),
    ('IS', 'Iceland', true),
    ('IN', 'India', true),
    ('ID', 'Indonesia', true),
    ('IR', 'Iran (Islamic Republic of)', true),
    ('IQ', 'Iraq', true),
    ('IE', 'Ireland', true),
    ('IM', 'Isle of Man', true),
    ('IL', 'Israel', true),
    ('IT', 'Italy', true),
    ('JM', 'Jamaica', true),
    ('JP', 'Japan', true),
    ('JE', 'Jersey', true),
    ('JO', 'Jordan', true),
    ('KZ', 'Kazakhstan', true),
    ('KE', 'Kenya', true),
    ('KI', 'Kiribati', true),
    ('KP', 'Korea (the Democratic People''s Republic of)', true),
    ('KR', 'Korea (the Republic of)', true),
    ('KW', 'Kuwait', true),
    ('KG', 'Kyrgyzstan', true),
    ('LA', 'Lao People''s Democratic Republic (the)', true),
    ('LV', 'Latvia', true),
    ('LB', 'Lebanon', true),
    ('LS', 'Lesotho', true),
    ('LR', 'Liberia', true),
    ('LY', 'Libya', true),
    ('LI', 'Liechtenstein', true),
    ('LT', 'Lithuania', true),
    ('LU', 'Luxembourg', true),
    ('MO', 'Macao', true),
    ('MG', 'Madagascar', true),
    ('MW', 'Malawi', true),
    ('MY', 'Malaysia', true),
    ('MV', 'Maldives', true),
    ('ML', 'Mali', true),
    ('MT', 'Malta', true),
    ('MH', 'Marshall Islands (the)', true),
    ('MQ', 'Martinique', true),
    ('MR', 'Mauritania', true),
    ('MU', 'Mauritius', true),
    ('YT', 'Mayotte', true),
    ('MX', 'Mexico', true),
    ('FM', 'Micronesia (Federated States of)', true),
    ('MD', 'Moldova (the Republic of)', true),
    ('MC', 'Monaco', true),
    ('MN', 'Mongolia', true),
    ('ME', 'Montenegro', true),
    ('MS', 'Montserrat', true),
    ('MA', 'Morocco', true),
    ('MZ', 'Mozambique', true),
    ('MM', 'Myanmar', true),
    ('NA', 'Namibia', true),
    ('NR', 'Nauru', true),
    ('NP', 'Nepal', true),
    ('NL', 'Netherlands (Kingdom of the)', true),
    ('NC', 'New Caledonia', true),
    ('NZ', 'New Zealand', true),
    ('NI', 'Nicaragua', true),
    ('NE', 'Niger (the)', true),
    ('NG', 'Nigeria', true),
    ('NU', 'Niue', true),
    ('NF', 'Norfolk Island', true),
    ('MK', 'North Macedonia', true),
    ('MP', 'Northern Mariana Islands (the)', true),
    ('NO', 'Norway', true),
    ('OM', 'Oman', true),
    ('PK', 'Pakistan', true),
    ('PW', 'Palau', true),
    ('PS', 'Palestine, State of', true),
    ('PA', 'Panama', true),
    ('PG', 'Papua New Guinea', true),
    ('PY', 'Paraguay', true),
    ('PE', 'Peru', true),
    ('PH', 'Philippines (the)', true),
    ('PN', 'Pitcairn', true),
    ('PL', 'Poland', true),
    ('PT', 'Portugal', true),
    ('PR', 'Puerto Rico', true),
    ('QA', 'Qatar', true),
    ('RO', 'Romania', true),
    ('RU', 'Russian Federation (the)', true),
    ('RW', 'Rwanda', true),
    ('RE', 'Réunion', true),
    ('BL', 'Saint Barthélemy', true),
    ('SH', 'Saint Helena, Ascension and Tristan da Cunha', true),
    ('KN', 'Saint Kitts and Nevis', true),
    ('LC', 'Saint Lucia', true),
    ('MF', 'Saint Martin (French part)', true),
    ('PM', 'Saint Pierre and Miquelon', true),
    ('VC', 'Saint Vincent and the Grenadines', true),
    ('WS', 'Samoa', true),
    ('SM', 'San Marino', true),
    ('ST', 'Sao Tome and Principe', true),
    ('SA', 'Saudi Arabia', true),
    ('SN', 'Senegal', true),
    ('RS', 'Serbia', true),
    ('SC', 'Seychelles', true),
    ('SL', 'Sierra Leone', true),
    ('SG', 'Singapore', true),
    ('SX', 'Sint Maarten (Dutch part)', true),
    ('SK', 'Slovakia', true),
    ('SI', 'Slovenia', true),
    ('SB', 'Solomon Islands', true),
    ('SO', 'Somalia', true),
    ('ZA', 'South Africa', true),
    ('GS', 'South Georgia and the South Sandwich Islands', true),
    ('SS', 'South Sudan', true),
    ('ES', 'Spain', true),
    ('LK', 'Sri Lanka', true),
    ('SD', 'Sudan (the)', true),
    ('SR', 'Suriname', true),
    ('SJ', 'Svalbard and Jan Mayen', true),
    ('SE', 'Sweden', true),
    ('CH', 'Switzerland', true),
    ('SY', 'Syrian Arab Republic (the)', true),
    ('TW', 'Taiwan (Province of China)', true),
    ('TJ', 'Tajikistan', true),
    ('TZ', 'Tanzania, the United Republic of', true),
    ('TH', 'Thailand', true),
    ('TL', 'Timor-Leste', true),
    ('TG', 'Togo', true),
    ('TK', 'Tokelau', true),
    ('TO', 'Tonga', true),
    ('TT', 'Trinidad and Tobago', true),
    ('TN', 'Tunisia', true),
    ('TM', 'Turkmenistan', true),
    ('TC', 'Turks and Caicos Islands (the)', true),
    ('TV', 'Tuvalu', true),
    ('TR', 'Türkiye', true),
    ('UG', 'Uganda', true),
    ('UA', 'Ukraine', true),
    ('AE', 'United Arab Emirates (the)', true),
    ('GB', 'United Kingdom of Great Britain and Northern Ireland (the)', true),
    ('UM', 'United States Minor Outlying Islands (the)', true),
    ('US', 'United States of America (the)', true),
    ('UY', 'Uruguay', true),
    ('UZ', 'Uzbekistan', true),
    ('VU', 'Vanuatu', true),
    ('VE', 'Venezuela (Bolivarian Republic of)', true),
    ('VN', 'Viet Nam', true),
    ('VG', 'Virgin Islands (British)', true),
    ('VI', 'Virgin Islands (U.S.)', true),
    ('WF', 'Wallis and Futuna', true),
    ('EH', 'Western Sahara*', true),
    ('YE', 'Yemen', true),
    ('ZM', 'Zambia', true),
    ('ZW', 'Zimbabwe', true),
    ('AX', 'Åland Islands', true)
ON CONFLICT (code)
DO UPDATE SET
    name = EXCLUDED.name,
    active = true;
